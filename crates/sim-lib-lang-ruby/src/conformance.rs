//! Ruby matrix-row conformance runner.
...
use crate::{ruby_dsl_matrix_row, ruby_dsl_profile};

/// Runs one Ruby DSL source conformance case.
pub fn run_ruby_dsl_conformance_case(
    _cx: &mut Cx,
    ...
    Ok(observe_profile_backed_case(
        case,
        &ruby_dsl_profile(),
        Symbol::qualified("ruby", "unsupported-source-case"),
        "case is outside Ruby profile descriptor coverage",
    ))
}

/// Runs the Ruby DSL matrix row and publishes claim-backed cells.
pub fn run_ruby_dsl_matrix_row(cx: &mut Cx) -> Result<MatrixRunReport> {
    let row = ruby_dsl_matrix_row();
    let report = MatrixRunner::run_source_row(cx, &row, run_ruby_dsl_conformance_case);
...
mod tests {
    ...
    #[test]
    fn ruby_dsl_matrix_row_runner_reports_profile_pass_and_runtime_gap() {
        let mut cx = cx();
        cx.grant(standard_test_capability());

        let report = run_ruby_dsl_matrix_row(&mut cx).unwrap();

        assert_eq!(report.cells.len(), 2);
        assert_eq!(report.pass_count(), 0);
        assert_eq!(report.gap_count(), 0);
        assert_eq!(report.fail_count(), 0);
        assert_eq!(report.language_fidelity(&Symbol::new("ruby")), None);
    }
}
