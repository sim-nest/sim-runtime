...
pub use linker::{
    ...
    LambdaMethodPipeline, LambdaProtocolTail, LambdaSerializationError, LinkageCache,
    LinkageFailure, LinkageState, LocatedJvmAdaptation, ManagedLambdaFactory,
    ManagedLambdaInstance, MethodIdentity, ResolvedBootstrapArgument, ResolvedDirectHandle,
    SelectedLambdaMember, SiteKey, StatelessLambdaIdentity, compile_jvm_function_plan,
    decode_lambda_bootstrap, discover_functional_interface, executor_admitted_lambda_protocols,
    invoke_lambda_member, resolve_direct_handle, validate_functional_interface,
};
