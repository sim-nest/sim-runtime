...
const GENERATED_SUPERINSTRUCTIONS: &str = "src/superinstructions_generated.rs";
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=superinstructions.toml");
    println!("cargo:rerun-if-changed={GENERATED_SUPERINSTRUCTIONS}");
    ...
    let generated = generate_superinstructions();
    if env::var_os("SIM_REGENERATE_JVM_SUPERINSTRUCTIONS").is_some() {
        fs::write(GENERATED_SUPERINSTRUCTIONS, &generated)
            .expect("write generated JVM superinstructions");
    } else {
        let checked = fs::read_to_string(GENERATED_SUPERINSTRUCTIONS).expect(
            "read generated superinstructions; regenerate with SIM_REGENERATE_JVM_SUPERINSTRUCTIONS=1 cargo check -p sim-lib-lang-jvm",
        );
        assert_eq!(
            checked, generated,
            "generated superinstructions are stale or hand-edited"
        );
    }
}

fn generate_superinstructions() -> String {
    let source =
        fs::read_to_string("superinstructions.toml").expect("read superinstruction manifest");
    let manifest: Value = source.parse().expect("parse superinstruction manifest");
    assert_eq!(
        manifest["schema"].as_str(),
        Some("sim.jvm-superinstructions/v1")
    );
    let definitions = manifest["superinstruction"]
        .as_array()
        .expect("superinstruction array");
    assert!(
        !definitions.is_empty() && definitions.len() <= 8,
        "fusion family must be bounded"
    );
    let mut names = std::collections::BTreeSet::new();
    let mut output =
        String::from("// @generated by build.rs from superinstructions.toml; DO NOT EDIT.\n\n");
    output.push_str("/// Generated fused-handler identity.\n#[derive(Clone, Copy, Debug, Eq, PartialEq)]\npub enum FusedHandler {\n");
    for definition in definitions {
        let name = definition["name"].as_str().expect("superinstruction name");
        assert!(names.insert(name), "duplicate superinstruction {name}");
        output.push_str(&format!(
            "    /// Profile-selected `{name}` sequence.\n    {name},\n"
        ));
    }
    ...
    output.push_str("/// Bounded fusion family, in deterministic manifest order.\npub const FUSED_DEFINITIONS: &[FusedDefinition] = &[\n");
    for definition in definitions {
        let name = definition["name"].as_str().unwrap();
        assert!(
            definition["profile_count"]
                .as_integer()
                .is_some_and(|count| count > 0),
            "{name} lacks measured frequency"
        );
        let opcodes = definition["opcodes"].as_array().expect("opcode array");
        assert!((2..=4).contains(&opcodes.len()), "{name} is not bounded");
        output.push_str(&format!(
            "    FusedDefinition {{ handler: FusedHandler::{name}, opcodes: &["
        ));
        for opcode in opcodes {
            let mnemonic = opcode.as_str().expect("opcode mnemonic");
            let metadata = sim_codec_classfile::OPCODES
                .iter()
                .find(|row| row.mnemonic == mnemonic)
                .unwrap_or_else(|| panic!("unknown opcode mnemonic {mnemonic}"));
            output.push_str(&format!("Opcode::{:?}, ", metadata.opcode));
        }
        output.push_str("] },\n");
    }
    output.push_str("];\n");
