...
pub fn decode_cl_lite_tree(
    ...
    let tree = parse_cl_lite_source(cx.codec, source_id, &source, &mut budget)?;
...
pub fn parse_cl_lite_source(
    ...
    if !parser.is_eof() {
        return parser.err("expected exactly one top-level CL-lite expression");
    }
    Ok(tree)
}
...
impl Parser<'_, '_> {
    fn read_expr(&mut self, depth: usize) -> Result<LocatedExprTree> {
        ...
        let Some(byte) = self.peek() else {
            return self.err("expected CL-lite expression");
        };
        match byte {
    ...
    fn read_list(&mut self, depth: usize, start: usize) -> Result<LocatedExprTree> {
        ...
        loop {
            ...
            match self.peek() {
                ...
                None => return self.err("unterminated CL-lite list"),
            }
        }
        self.budget
    ...
    fn read_string(&mut self, start: usize) -> Result<LocatedExprTree> {
        ...
        self.err("unterminated CL-lite string")
    }

    fn read_escape(&mut self) -> Result<char> {
        let Some(escaped) = self.peek() else {
            return self.err("unterminated CL-lite string escape");
        };
        self.index += 1;
        Ok(match escaped {
            b'n' => '\n',
    ...
    fn read_atom(&mut self, start: usize) -> Result<LocatedExprTree> {
        ...
        if atom.is_empty() {
            return self.err("expected CL-lite atom");
        }
...
fn symbol_atom(atom: &str) -> Symbol {
    ...
    if let Some((package, name)) = atom.split_once("::") {
        return Symbol::qualified(package.to_owned(), name.to_owned());
    }
...
fn input_text(codec: CodecId, input: Input) -> Result<String> {
    match input {
        ...
        Input::Bytes(bytes) => String::from_utf8(bytes).map_err(|err| Error::CodecError {
            codec,
            message: format!("CL-lite input is not valid UTF-8: {err}"),
        }),
    }
}
