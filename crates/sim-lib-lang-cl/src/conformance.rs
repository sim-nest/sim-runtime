//! Common Lisp lite matrix-row conformance runner.
...
use crate::{cl_lite_matrix_row, cl_lite_profile};

/// Runs one Common Lisp lite source conformance case.
pub fn run_cl_lite_conformance_case(
    _cx: &mut Cx,
    ...
    Ok(observe_profile_backed_case(
        case,
        &cl_lite_profile(),
        Symbol::qualified("common-lisp", "unsupported-source-case"),
        "case is outside CL-lite profile descriptor coverage",
    ))
}

/// Runs the Common Lisp lite matrix row and publishes claim-backed cells.
pub fn run_cl_lite_matrix_row(cx: &mut Cx) -> Result<MatrixRunReport> {
    let row = cl_lite_matrix_row();
    let report = MatrixRunner::run_source_row(cx, &row, run_cl_lite_conformance_case);
...
mod tests {
    ...
    #[test]
    fn cl_lite_matrix_row_runner_reports_profile_pass_and_runtime_gap() {
        let mut cx = cx();
        cx.grant(standard_test_capability());

        let report = run_cl_lite_matrix_row(&mut cx).unwrap();

        assert_eq!(report.cells.len(), 2);
        assert_eq!(report.pass_count(), 0);
        assert_eq!(report.gap_count(), 0);
        assert_eq!(report.fail_count(), 0);
        assert_eq!(report.language_fidelity(&Symbol::new("common-lisp")), None);
    }
}
