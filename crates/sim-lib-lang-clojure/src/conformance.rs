//! Clojure matrix-row conformance runner.
...
use crate::{clojure_core_matrix_row, clojure_core_profile};

/// Runs one Clojure core source conformance case.
pub fn run_clojure_core_conformance_case(
    _cx: &mut Cx,
    ...
    Ok(observe_profile_backed_case(
        ...
        Symbol::qualified("clojure", "unsupported-source-case"),
        "case is outside Clojure profile descriptor coverage",
    ))
}

/// Runs the Clojure core matrix row and publishes claim-backed cells.
pub fn run_clojure_core_matrix_row(cx: &mut Cx) -> Result<MatrixRunReport> {
    let row = clojure_core_matrix_row();
    let report = MatrixRunner::run_source_row(cx, &row, run_clojure_core_conformance_case);
...
mod tests {
    ...
    #[test]
    fn clojure_core_matrix_row_runner_reports_profile_pass_and_runtime_gap() {
        let mut cx = cx();
        cx.grant(standard_test_capability());

        let report = run_clojure_core_matrix_row(&mut cx).unwrap();

        assert_eq!(report.cells.len(), 2);
        assert_eq!(report.pass_count(), 0);
        assert_eq!(report.gap_count(), 0);
        assert_eq!(report.fail_count(), 0);
        assert_eq!(report.language_fidelity(&Symbol::new("clojure")), None);
    }
}
