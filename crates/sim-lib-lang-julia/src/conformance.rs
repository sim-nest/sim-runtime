//! Julia matrix-row conformance runner.
...
use crate::{julia_core_matrix_row, julia_core_profile};

/// Runs one Julia core source conformance case.
pub fn run_julia_core_conformance_case(
    _cx: &mut Cx,
    ...
    Ok(observe_profile_backed_case(
        case,
        &julia_core_profile(),
        Symbol::qualified("julia", "unsupported-source-case"),
        "case is outside Julia profile descriptor coverage",
    ))
}

/// Runs the Julia core matrix row and publishes claim-backed cells.
pub fn run_julia_core_matrix_row(cx: &mut Cx) -> Result<MatrixRunReport> {
    let row = julia_core_matrix_row();
    let report = MatrixRunner::run_source_row(cx, &row, run_julia_core_conformance_case);
...
mod tests {
    ...
    #[test]
    fn julia_core_matrix_row_runner_reports_profile_pass_and_runtime_gap() {
        let mut cx = cx();
        cx.grant(standard_test_capability());

        let report = run_julia_core_matrix_row(&mut cx).unwrap();

        assert_eq!(report.cells.len(), 2);
        assert_eq!(report.pass_count(), 0);
        assert_eq!(report.gap_count(), 0);
        assert_eq!(report.fail_count(), 0);
        assert_eq!(report.language_fidelity(&Symbol::new("julia")), None);
    }
}
